...
const scenarioOptions: { id: ScenarioId; label: string; detail: string }[] = [
  ...
  { id: 'appointment', label: 'Assessment scheduled', detail: 'Upcoming demo appointment' },
...
export function DashboardPage() {
  ...
  return <div className="container page-section">
    <div className="demo-toolbar" aria-labelledby="demo-scenario-heading"><div><p className="eyebrow" id="demo-scenario-heading">Demo scenario</p><p>Switch states to see a coherent application, timeline and next action.</p></div><select aria-label="Choose demo scenario" value={activeScenario} onChange={(e) => setActiveScenario(e.target.value as ScenarioId)}>{scenarioOptions.map((option) => <option key={option.id} value={option.id}>{option.label} — {option.detail}</option>)}</select><button className="text-button" onClick={reset}>Reset demo data</button></div>
    <Breadcrumbs items={[{ label: 'Home', to: '/' }, { label: 'Your UDID services' }]} />
    <PageIntro eyebrow={app.id} title={`Hello, ${app.applicantName}`}><p>This dashboard shows the complete synthetic record for the selected scenario.</p></PageIntro>
    ...
    <div className="dashboard-grid">
      ...
      <aside className="dashboard-side">
        ...
        {app.currentStatus === 'CARD_DISPATCHED' && <section className="document-card"><p className="eyebrow">Document ready</p><h2>Synthetic certificate</h2><p>Open the labelled prototype document and download a text copy.</p><Link className="primary-button" to="/documents/certificate">View certificate</Link></section>}
...
export function CorrectionPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!fileName) { setError('Choose a safe synthetic file or use the demo filename.'); return }
    correctDocument(fileName)
    setComplete(true)
  }

  const openDashboard = () => { setActiveScenario('correction'); navigate('/dashboard') }
  return <div className="container narrow page-section"><Breadcrumbs items={[{ label: 'Dashboard', to: '/dashboard' }, { label: 'Correct application' }]} /><PageIntro eyebrow={app.id} title={complete ? 'Correction submitted' : 'Your application needs a correction'}><p>{complete ? 'The corrected document is now in the demo review state.' : 'Only the address proof needs attention. The rest of the application is saved.'}</p></PageIntro>
    {complete ? <><Alert type="success" title="Corrected document received">Version 2 was added and a timeline event, notification and audit-style state update were created together.</Alert><button className="primary-button" onClick={openDashboard}>View updated dashboard</button></> : <>
      <section className="rejection-panel"><p className="eyebrow">What needs attention</p><h2>Address proof could not be verified</h2><p>Please upload a clearer synthetic document where the name and address would be readable. Your other documents and answers are saved.</p><dl><div><dt>Document</dt><dd>{problem?.displayName}</dd></div><div><dt>Current version</dt><dd>Version {problem?.version}</dd></div><div><dt>Reason</dt><dd>Image is too unclear to read</dd></div></dl></section>
      <form onSubmit={submit} className="correction-form"><h2>Upload corrected address proof</h2><p className="hint">PDF, JPG or PNG · maximum 2 MB. This prototype stores the filename only.</p><Alert type="warning" title="Use a synthetic file">Do not select a real address proof or personal document.</Alert><input aria-label="Corrected synthetic address proof" type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => { setFileName(e.target.files?.[0]?.name || ''); setError('') }} />{fileName && <p><strong>Selected:</strong> {fileName}</p>}{error && <p className="field-error" role="alert">{error}</p>}<div className="button-row"><button type="button" className="text-button" onClick={() => { setFileName('corrected-synthetic-address-proof.pdf'); setError('') }}>Use safe demo filename</button><button className="primary-button" type="submit">Submit corrected document</button></div></form>
...
export function AppointmentPage() {
  ...
  return <div className="container narrow page-section"><Breadcrumbs items={[{ label: 'Dashboard', to: '/dashboard' }, { label: 'Appointment details' }]} /><PageIntro title="Your synthetic assessment appointment"><p>Review the time, place, accessibility information and preparation checklist.</p></PageIntro>{saved && <Alert type="success" title="Demo appointment updated">The application timeline and notification centre were updated too.</Alert>}<section className="appointment-detail"><p className="eyebrow">Upcoming appointment</p><h2>{saved ? date : appointment.date} at {saved ? time : appointment.time}</h2><h3>{appointment.locationName}</h3><p>{appointment.address}</p><dl><div><dt>Accessibility</dt><dd>{appointment.accessNotes}</dd></div><div><dt>Bring</dt><dd>Synthetic appointment letter and synthetic documents used in the application</dd></div><div><dt>Arrival</dt><dd>20 minutes early — demo guidance only</dd></div></dl></section>{editing ? <form className="reschedule-form" onSubmit={submit}><h2>Choose another demo time</h2><div className="field"><label htmlFor="new-date">Date</label><select id="new-date" value={date} onChange={(e) => setDate(e.target.value)}><option>8 September 2026</option><option>10 September 2026</option></select></div><div className="field"><label htmlFor="new-time">Time</label><select id="new-time" value={time} onChange={(e) => setTime(e.target.value)}><option>11:45 AM</option><option>2:15 PM</option></select></div><div className="button-row"><button type="button" className="text-button" onClick={() => setEditing(false)}>Cancel</button><button className="primary-button">Confirm demo appointment</button></div></form> : <button className="secondary-button" onClick={() => setEditing(true)}>Reschedule demo appointment</button>}<Alert title="This is not a real booking">No medical centre was contacted and no official appointment was changed.</Alert></div>
}
...
export function CertificatePage() {
  ...
  const download = () => {
    const content = `SYNTHETIC UDID CERTIFICATE\n\nPrototype only — not an official document\nApplication: ${app.id}\nApplicant: ${app.applicantName}\nStatus: ${statusLabels[app.currentStatus]}\n\nThis file contains synthetic data and has no legal validity.`
    const url = URL.createObjectURL(new Blob([content], { type: 'text/plain' }))
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = 'synthetic-udid-certificate.txt'
    anchor.click()
    URL.revokeObjectURL(url)
  }
  return <div className="container narrow page-section"><Breadcrumbs items={[{ label: 'Dashboard', to: '/dashboard' }, { label: 'Synthetic certificate' }]} /><PageIntro title="Your synthetic certificate"><p>Preview and download a clearly labelled prototype document.</p></PageIntro><Alert type="warning" title="Not an official certificate">This document has no legal validity and cannot be used for any government or identity purpose.</Alert><article className="certificate"><div className="certificate-banner">SYNTHETIC · PROTOTYPE ONLY</div><p className="eyebrow">UDID service redesign</p><h2>Demo disability certificate</h2><dl><div><dt>Applicant</dt><dd>{app.applicantName}</dd></div><div><dt>Application reference</dt><dd>{app.id}</dd></div><div><dt>Prototype status</dt><dd>Certificate generated</dd></div><div><dt>Generated</dt><dd>26 August 2026</dd></div></dl><p className="certificate-note">This is a synthetic hackathon artifact. It is not issued, signed or endorsed by any government authority.</p></article><div className="button-row"><button className="primary-button" onClick={download}>Download synthetic certificate</button><Link className="secondary-button" to="/dashboard">Back to dashboard</Link></div></div>
}
