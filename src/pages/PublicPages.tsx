...
function ServiceTaskPage({ kind }: { kind: 'renew' | 'replace' }) {
  ...
  return <div className="container narrow page-section">
    ...
    <figure className="journey-photo-card service-page-photo card-service-photo"><img src={renewal ? '/assets/service-dashboard-consultation.jpg' : '/assets/service-application-documents.jpg'} alt={renewal ? 'A clinician and visitor discussing information in a consultation room' : 'Hands completing forms at a desk'} /><figcaption><strong>{renewal ? 'Keep your record moving forward' : 'Replace only what is needed'}</strong><span>{renewal ? 'Review your existing details and request the next stage without losing your history.' : 'A clear, focused request keeps your existing application history connected.'}</span></figcaption></figure>
