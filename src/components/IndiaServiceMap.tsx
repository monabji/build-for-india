...
const states: StateInfo[] = [
  { name: 'Jammu and Kashmir', x: 174, y: 70, region: 'North', centres: 3, languages: 'English, Hindi, Urdu', guidance: 'Assisted-service desks are shown for this demo region.' },
  { name: 'Punjab', x: 145, y: 123, region: 'North', centres: 5, languages: 'Punjabi, Hindi, English', guidance: 'Choose a demo district to see accessible assessment centres.' },
  { name: 'Rajasthan', x: 112, y: 195, region: 'West', centres: 8, languages: 'Hindi, English', guidance: 'Low-bandwidth application support is available in this demo.' },
  { name: 'Gujarat', x: 76, y: 267, region: 'West', centres: 7, languages: 'Gujarati, Hindi, English', guidance: 'A caregiver can begin and save an application in this demo.' },
  { name: 'Maharashtra', x: 145, y: 300, region: 'West', centres: 12, languages: 'Marathi, Hindi, English', guidance: 'Three synthetic centres include step-free access information.' },
  { name: 'Karnataka', x: 150, y: 365, region: 'South', centres: 9, languages: 'Kannada, English, Hindi', guidance: 'Appointment and document guidance is available in this demo.' },
  ...
  { name: 'Tamil Nadu', x: 218, y: 420, region: 'South', centres: 10, languages: 'Tamil, English', guidance: 'Synthetic service guidance is available in Tamil and English.' },
  { name: 'Telangana', x: 205, y: 326, region: 'South', centres: 7, languages: 'Telugu, Urdu, English', guidance: 'Find a synthetic centre or start a guided application.' },
...
export function IndiaServiceMap({ compact = false }: { compact?: boolean }) {
  ...
  const [announcement, setAnnouncement] = useState('Maharashtra selected. 12 demo centres available.')

  const choose = (state: StateInfo) => {
    setSelected(state)
    setAnnouncement(`${state.name} selected. ${state.centres} demo centres available.`)
  }
  ...
  return <section className={`india-finder ${compact ? 'india-finder-compact' : ''}`} aria-labelledby="india-finder-title">
    <div className="finder-heading">
      <div><p className="eyebrow">Start with where you live</p><h2 id="india-finder-title">Find services across India</h2><p>Select a state to see synthetic service availability, language support and the next useful action.</p></div>
    ...
    <div className="finder-body">
      {view === 'map' ? <div className="map-stage">
        <svg className="india-map" viewBox="0 0 420 520" role="img" aria-labelledby="map-title map-description">
          ...
          <desc id="map-description">Choose one of twelve demo states. A standard select and list provide the same actions.</desc>
          ...
          {states.map((state) => <g key={state.name} className={`state-node ${selected.name === state.name ? 'selected' : ''}`} role="button" tabIndex={0} aria-label={`${state.name}, ${state.centres} demo centres`} aria-current={selected.name === state.name ? 'true' : undefined} onClick={() => choose(state)} onKeyDown={(event) => activateFromKeyboard(event, state)}>
        ...
        <p className="map-note">Illustrative prototype map · use the state list for the complete accessible route</p>
      </div> : <div className="state-list" aria-label="Demo states">{states.map((state) => <button key={state.name} type="button" className={selected.name === state.name ? 'selected' : ''} aria-pressed={selected.name === state.name} onClick={() => choose(state)}><span><strong>{state.name}</strong><small>{state.region} · {state.languages}</small></span><span>{state.centres} centres</span></button>)}</div>}
      <aside className="state-result" key={selected.name} aria-labelledby="selected-state-title">
        ...
        <p className="centre-count"><strong>{selected.centres}</strong> synthetic service centres</p>
        <dl><div><dt>Languages</dt><dd>{selected.languages}</dd></div><div><dt>Demo guidance</dt><dd>{selected.guidance}</dd></div></dl>
